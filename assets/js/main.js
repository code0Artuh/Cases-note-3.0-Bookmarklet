...
const casesNotes = () => {
  ...
  const showPopup = (data) => {
    ...
    fetch(
      `https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/html/template_popup/${data}.html`,
    )
      .then((response) => {
  ...
  const createEmail = (hotkeyString) => {
    ...
    const functionCreateEmail = () => {
      ...
      setTimeout(() => {
        const buttonCreateEmail = document.querySelector(
            ...
            if (elements.length > 0) {
              ...
              setTimeout(() => {
                ...
                const clickHotKey = new MutationObserver((mutations) => {
                  mutations.forEach((mutation) => {
                    if (mutation.target.innerText.includes('1 match')) {
                      mutation.target
                        .querySelector('material-select-dropdown-item')
                        .dispatchEvent(bubbleEventClick);
  ...
  const handleEmailAutomateButtonClick = (e) => {
    ...
    const templateHTML = fetch(
      `https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/html/template_email/${dataEmail}.html`,
    ).then((e) => e.text());
  ...
  const dadosQa = fetch(
    'https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/js/dadosqa.json',
  ).then((e) => e.json());
  dadosQa
    .then((data) => {
  ...
  const alertsContent = fetch(
    'https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/html/template_alert_content/tmplate_alert.html',
  ).then((e) => e.text());
  alertsContent
    .then((data) => {
  ...
  createStyle('https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/css/style.css');
...
const structureHTML = fetch(
  'https://code0artuh.github.io/Cases-note-3.0-Bookmarklet/assets/html/estrutura.html',
).then((e) => e.text());
